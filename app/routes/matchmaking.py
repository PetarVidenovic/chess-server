    ...
    async with matchmaking_lock:
        ...
        while i < len(matchmaking_queue) - 1:
            ...
            if abs(p1_rating - p2_rating) <= 200:
                # Ukloni obojicu iz reda
                matchmaking_queue.pop(i)
                matchmaking_queue.pop(i)  # sada je i+1 postao i
                game_id = str(uuid.uuid4())
                # Opciono: sačuvaj igru u bazu
                # await save_game_to_db(p1_id, p2_id, game_id)
